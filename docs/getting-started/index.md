---
layout: default
title: Getting Started
nav_order: 2
has_children: true
permalink: /getting-started
---

# Getting Started

Essential resources to begin using XDL. Start here if you're new to XDL.

## Quick Links

- [Quick Start Guide](quick-start) - Get up and running in minutes
- [Installation](installation) - Detailed installation instructions
- [GPU Acceleration Demo](gpu-demo) - GPU features walkthrough
- [Graphics Quick Start](graphics-quickstart) - 2D graphics basics
- [3D Visualization Quick Start](viz3d-quickstart) - 3D visualization basics
- [Examples](examples) - Sample code and tutorials

## What is XDL?

XDL (Extended Data Language) is a modern Rust implementation providing IDL-compatible data analysis and visualization capabilities. It offers:

- **Memory Safety**: Leveraging Rust's ownership system
- **High Performance**: Native code with GPU acceleration
- **IDL/MATLAB Compatibility**: Run existing scientific code
- **Modern Features**: Python integration, advanced visualization

## First Steps

1. **Install XDL** - Follow the [installation guide](installation)
2. **Try the Quick Start** - Run your first XDL program
3. **Explore Examples** - Learn from sample code
4. **Read the Guides** - Dive into specific features
