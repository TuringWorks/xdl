        ...
        for layer in &u {
            if layer.len() != dims.1 {
                return Err(XdlError::InvalidArgument(
                    "Inconsistent dimensions".to_string(),
                ));
            }
            for row in layer {
                if row.len() != dims.2 {
...
impl VectorField2D {
    ...
    pub fn get_magnitude(&self, i: usize, j: usize) -> Option<f64> {
        self.get_vector(i, j).map(|(u, v)| (u * u + v * v).sqrt())
    }
}
...
pub fn integrate_streamline(
    ...
    for _ in 0..max_steps {
        ...
        let (k2_u, k2_v) = interpolate_vector(
            field,
            x + 0.5 * step_size * k1_u,
            y + 0.5 * step_size * k1_v,
        );
        let (k3_u, k3_v) = interpolate_vector(
            field,
            x + 0.5 * step_size * k2_u,
            y + 0.5 * step_size * k2_v,
        );
