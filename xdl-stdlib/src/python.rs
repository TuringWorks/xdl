...
mod tests {
    ...
    #[test]
    fn test_python_import_no_args() {
        let args = vec![];
        let result = python_import(&args);
        assert!(result.is_err());
    }

    #[test]
    fn test_python_import_invalid_type() {
    ...
    fn test_python_call_no_args() {
        let args = vec![];
        let result = python_call(&args);
        assert!(result.is_err());
    }

    #[test]
    fn test_python_call_insufficient_args() {
        let args = vec![XdlValue::String("module".to_string())];
        let result = python_call(&args);
        assert!(result.is_err());
    }

    #[test]
    fn test_python_call_invalid_module_type() {
        let args = vec![XdlValue::Long(42), XdlValue::String("method".to_string())];
        let result = python_call(&args);
        assert!(result.is_err());
    }

    #[test]
    fn test_python_call_invalid_method_type() {
        let args = vec![XdlValue::String("module".to_string()), XdlValue::Long(42)];
        let result = python_call(&args);
        assert!(result.is_err());
    }

    #[test]
    fn test_python_call_kw_no_args() {
        let args = vec![];
        let result = python_call_kw(&args);
        assert!(result.is_err());
    }

    #[test]
    fn test_python_call_kw_insufficient_args() {
        let args = vec![XdlValue::String("module".to_string())];
        let result = python_call_kw(&args);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_python_call_kw_invalid_module_type() {
        let args = vec![XdlValue::Long(42), XdlValue::String("method".to_string())];
        let result = python_call_kw(&args);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_python_call_kw_invalid_method_type() {
        let args = vec![XdlValue::String("module".to_string()), XdlValue::Long(42)];
        let result = python_call_kw(&args);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_python_call_kw_with_keyword_args() {
        let args = vec![
            XdlValue::String("module".to_string()),
            XdlValue::String("method".to_string()),
            XdlValue::String("key1=42".to_string()),
            XdlValue::String("key2=3.14".to_string()),
        ];
        // This will likely fail due to no Python manager, but should parse args correctly
        let result = python_call_kw(&args);
        // We expect this to fail since there's no actual Python integration set up
        // but it should fail gracefully, not with a parsing error
        assert!(result.is_err());
    }
}
